...
if($page<1) $page=1;
$pagerow=20;
$query_count=$dblink->query("select count(*) from {$dbprefix}ques where title like '%$word%'");
$quescount=$dblink->result($query_count,0); 
$pagecount=ceil($quescount/$pagerow);
if($page>$pagecount) $page=1;
$pagestart=($page-1)*$pagerow;
$query=$dblink->query("select qid,title,status from {$dbprefix}ques where title like '%$word%' order by qid desc limit $pagestart,$pagerow");
$i=1;
