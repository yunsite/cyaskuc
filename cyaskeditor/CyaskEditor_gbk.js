...
str+='<iframe name="CyaskImageIframe" id="CyaskImageIframe" frameborder="0" style="width:228px;height:308px;padding:0;margin:0;border:0;">';
str+='</iframe></div>';
return str;
break;
case 'CE_LINK':
...
switch(cmd){case 'CE_IMAGE':
var str='<table cellpadding="0" cellspacing="0" style="width:100%;font-size:12px;">'+
'<tr><td colspan="2"><table border="0"><tr><td id="imagePreview" style="width:220px;height:220px;border:1px solid #AAAAAA;background-color:#FFFFFF;" align="center" valign="middle">&nbsp;</td></tr></table></td></tr>'+
'<tr><td style="width:30px;padding:5px;">'+CE_LANG['REMOTE']+'</td>'+
'<td style="width:190px;padding-bottom:5px;"><input type="text" id="imgLink" value="http://" maxlength="255" style="width:180px;border:1px solid #555555;" />'+
'<input type="hidden" name="imageType" id="imageType" value="2"><input type="hidden" name="fileData" id="imgFile" value="" /></td></tr>'+
'<tr><td colspan="2" style="margin:5px;padding-bottom:5px;" align="center">'+
CE_LANG['WIDTH']+'&nbsp;<input type="text" name="imgWidth" id="imgWidth" value="0" maxlength="4" style="width:35px;border:1px solid #555555;" />&nbsp;'+
CE_LANG['HEIGHT']+'&nbsp;<input type="text" name="imgHeight" id="imgHeight" value="0" maxlength="4" style="width:35px;border:1px solid #555555;" /></td></tr>'+
'<tr><td colspan="2" style="margin:5px;padding-bottom:5px;" align="center">'+
'<input type="button" name="button" value="'+CE_LANG['PREVIEW']+'" onclick="javascript:parent.CyaskImagePreview();" style="border:1px solid #555555;background-color:'+CE_BUTTON_COLOR+';" /> '+
...
'<input type="submit" name="button" id="'+cmd+'submitButton" value="'+CE_LANG['CONFIRM']+'" onclick="javascript:parent.CyaskDrawLinkEnd();" style="border:1px solid #555555;background-color:'+CE_BUTTON_COLOR+';" /> '+
'<input type="button" name="button" value="'+CE_LANG['CANCEL']+'" onclick="javascript:parent.CyaskDisableMenu();" style="border:1px solid #555555;background-color:'+CE_BUTTON_COLOR+';" /></td></tr>';
str+='</table>';CyaskDrawMenuIframe(CE_LINK_DOCUMENT,str);break;
default:break;}}
function CyaskDrawMenuIframe(obj,str){obj.open();obj.write(str);obj.close();obj.body.style.color=CE_MENU_TEXT_COLOR;obj.body.style.backgroundColor=CE_MENU_BG_COLOR;obj.body.style.margin=0;obj.body.scroll='no';}
...
var type=CE_IMAGE_DOCUMENT.getElementById('imageType').value;
var file=CE_IMAGE_DOCUMENT.getElementById('imgFile').value;
var imgObj=CE_IMAGE_DOCUMENT.createElement("IMG");
imgObj.src=url;var width=parseInt(imgObj.width);var height=parseInt(imgObj.height);var rate=parseInt(width/height);
CE_IMAGE_DOCUMENT.getElementById('imgWidth').value=width;
CE_IMAGE_DOCUMENT.getElementById('imgHeight').value=height;
if(width>220||height>220){if(width>=height){width=220;height=parseInt(width/rate);}else{height=220;width=parseInt(height*rate);}}
imgObj.style.width=width;imgObj.style.height=height;var el=CE_IMAGE_DOCUMENT.getElementById('imagePreview');
if(el.hasChildNodes()){el.removeChild(el.childNodes[0]);}
el.appendChild(imgObj);return imgObj;}
function CyaskDrawImageEnd(){var url=CE_IMAGE_DOCUMENT.getElementById('imgLink').value;
if(CyaskCheckImageFileType(url,"/")==false){return false;}
CyaskEditorForm.focus();CyaskSelect();
var obj=document.createElement("img");obj.src=url;var width=parseInt(obj.width);var height=parseInt(obj.height);var rate=parseInt(width/height);
if(width>600||height>600){if(width>=height){width=600;height=parseInt(width/rate);}else{height=600;width=parseInt(height*rate);}}
obj.border=0;obj.width=width;obj.height=height;CyaskInsertItem(obj);CyaskDisableMenu();}
function CyaskDrawLinkEnd(){var range;var url=CE_LINK_DOCUMENT.getElementById('hyperLink').value;var target=CE_LINK_DOCUMENT.getElementById('hyperLinkTarget').value;
...
newRange.setStart(startRangeNode,startRangeOffset+insertNode.length);}else{var afterNode;
